...
class LlmPresetStore:
    ...
    def _summary(record: dict[str, Any]) -> LlmPresetSummary:
        return LlmPresetSummary(
            ...
            chunk_size_days=int(record.get("chunk_size_days", 3)),
            call_delay_seconds=float(record.get("call_delay_seconds", 2)),
            timeout_seconds=float(record.get("timeout_seconds", 120)),
            max_output_tokens=int(record.get("max_output_tokens", 5000)),
