...
export function AssetStressSentimentChart({
  ...
  const tooltipPoint =
    activePoint ||
    chartSeries.flatMap((assetSeries) => assetSeries.points).find((point) => point.date === selectedDate) ||
    null;
  ...
  return (
    <div className="asset-stress-chart" style={{ width: effectiveWidth }}>
      ...
      >
        <defs>
          <linearGradient id="asset-stress-grid-fade" x1="0" x2="1" y1="0" y2="0">
            <stop offset="0%" stopColor="#d9e0e6" stopOpacity="0.9" />
            <stop offset="100%" stopColor="#d9e0e6" stopOpacity="0.25" />
          </linearGradient>
        </defs>
        {[
