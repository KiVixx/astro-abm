...
.PHONY: help status bootstrap up db-up down migrate maintain-now market-series-maintain marksix-maintain astro-daily research-store research-prepare product-snapshots fetch-local-data smoke checkpoint checkpoint-check check-api-port check-web-port api web api-production web-build web-production product-smoke scenario-demo cleanup-guests security-status open-source-audit test
...
help:
	...
	@echo "  make marksix-maintain   Build or refresh the local Hong Kong Mark Six draw database"
...
marksix-maintain:
	uv run python scripts/maintain_marksix.py
