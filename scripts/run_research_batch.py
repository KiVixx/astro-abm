...
def main() -> int:
    ...
    validation_warnings = validate_exploratory_batch_outputs(ROOT / args.output)
    if args.ingest:
    ...
    print(f"run_id={batch.run_id} rows={len(batch.results)} output={paths['summary.md']} validation_warnings={len(validation_warnings)}")
