...
更新日期：2026-07-16
目前主線：`main`
目前定位：本地優先市場世界線推演平台 + 描述性歷史關聯研究，不是交易系統。

這份文件是給維護者快速接手用的中文簡報。文件會保留必要英文欄位或檔名，因為程式碼、資料表、報告仍使用英文命名；但每個重要英文詞會盡量附中文註釋，方便往後維護。

## 投影片 1：一句話定位

Astro ABM 目前不是「自動交易模型」，也不是已驗證能預測市場的完整
`agent-based model`（代理人基礎市場模型）。

它現在已經有可操作的本地 `Worldline Simulator`（世界線推演器）：可按日生成
代理群體事件、模擬因果鏈、壓力狀態、fallback（安全回退）、審查與從指定
chunk（分段）重建。這套引擎用於 scenario rehearsal（情境演練），不代表真實
因果或預測準確度。

底層仍是一套「資料工程 + 100 年研究輸出」系統，用來檢查：
...
| 世界線產品層 | Worldline product layer | 用日線脈絡、代理群體與可選 LLM 做逐日情境推演 | 1D | 建立、播放、審查與重建模擬世界線 |

簡單記法：

- `src/astro_abm/`：小時級資料工程主系統。
- `astro_research/`：日線級 100 年研究主系統。
- `apps/api/` + `apps/web/`：本地世界線 API、Web 工作台與報告系統。
